...
use llama_core::{ MetadataBuilder, init_core_context };
use anyhow::bail;
...
async fn main() -> anyhow::Result<()> {
    ...
    // n_predict
    log(format!("[INFO] Number of tokens to predict: {}", &cli.n_predict));
    // n_gpu_layers
    log(format!("[INFO] Number of layers to run on the GPU: {}", &cli.n_gpu_layers));
    // batch size
    log(format!("[INFO] Batch size for prompt processing: {}", &cli.batch_size));
    // temp and top_p
    ...
    // repeat penalty
    log(format!("[INFO] Penalize repeat sequence of tokens: {}", &cli.repeat_penalty));
    // presence penalty
    log(format!("[INFO] Presence penalty (0.0 = disabled): {}", &cli.presence_penalty));
    // frequency penalty
    log(format!("[INFO] Frequency penalty (0.0 = disabled): {}", &cli.frequency_penalty));
    // log prompts
